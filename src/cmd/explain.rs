...
fn registry() -> &'static [DiagnosticExplanation] {
    &[
        ...
        DiagnosticExplanation {
            ...
            common_causes: &["invalid YAML", "unknown policy field", "unreadable policy file"],
            fixes: &["fix the YAML syntax", "validate the file against traceability-policy-schema.json", "restore the policy generated by hlv init"],
        },
        DiagnosticExplanation {
            code: "TRC-003",
            title: "Test ID pattern is invalid",
            meaning: "The id_formats.test value in validation/traceability-policy.yaml is not a valid regular expression.",
            common_causes: &["unclosed character class or group", "invalid escape sequence", "regex copied through YAML quoting incorrectly"],
            fixes: &["correct the regular expression", "quote the YAML value safely", "remove id_formats.test when only built-in HLV prefixes are needed"],
        },
        DiagnosticExplanation {
            code: "TRC-022",
            ...
            common_causes: &["typo in traceability.yaml tests", "test was planned but not added to a test spec", "project.yaml contract entry points at the wrong test_spec file", "project-specific test ID does not match id_formats.test"],
            fixes: &["add the missing test ID to the relevant test spec", "fix the ID in traceability.yaml", "ensure the contract entry's test_spec path points to the file that declares the test", "update id_formats.test for the project's custom test ID convention"],
        },
        DiagnosticExplanation {
            code: "MAP-080",
