...
- **paths.code** - observed legacy source/test roots for adopted projects; these resolve from the repository root, not `.hlv/`
...
Adopt mode attaches HLV to an existing repository without moving code:

- HLV-owned files live under `.hlv/` (`.hlv/project.yaml`, `.hlv/milestones.yaml`, `.hlv/human/`, `.hlv/validation/`, `.hlv/llm/`).
- Root-owned files remain at the repository root (`AGENTS.md`, `HLV.md`, existing source/test files).
- `project.yaml -> paths.code` lists observed legacy source/test roots relative to the repository root.
- `project.yaml -> paths.llm` remains the generated-code home under `.hlv/`.
- `features.legacy_mode: true` relaxes marker enforcement for untouched legacy code; changed milestone files still re-enter the full contract, implementation, and validation flow.
- `.hlv/index/signatures.yaml` is generated by `hlv index build` and queried through `hlv index show/list --json` or the MCP index tools. `features.index_tracking` records whether the index is ignored or intentionally tracked.
...
**Data model.** `GatesPolicy` contains `Vec<Gate>`. Each `Gate` is a struct with fields `id`, `type`, `mandatory`, `enabled`, `command`, `cwd`, `pass_criteria`. `command` is a portable `executable + args` string (not a shell script). `cwd` is a project-relative path (not absolute, no `..` components) and must point to an existing directory when set or run. Serialization/deserialization uses serde.
...
**Persistence.** `GatesPolicy::save()` serializes the structure to YAML and writes it to `validation/gates-policy.yaml`. The path is taken from `project.yaml -> paths.validation`. All CRUD operations (`add`/`remove`/`edit`/`enable`/`disable`/`set-cmd`/`clear-cmd`/`set-cwd`/`clear-cwd`) call `save()` after mutation. `add --command` and `set-cmd` parse the portable command before saving; `add --cwd` and `set-cwd` verify the path is project-relative and the directory exists before saving.

**Execution.** `gates run [<id>]` parses `command` as `program + args` and executes it via `std::process::Command`. Working directory is `cwd` relative to the project root (or the project root if `cwd` is not set). Missing `cwd`, unsupported shell syntax (`&&`, `||`, `|`, `;`, redirection, shell variable expansion like `$VAR`/`${VAR}`/`$()`), parse errors, spawn failures, and non-zero exits are reported distinctly. Disabled gates and enabled gates without commands are counted as skipped. Exit code `0` = no failed gates. `hlv gates --json run [<id>]` returns `{ passed, failed, skipped, results }`, where each result includes `id`, `status`, `reason`, `cwd`, and `command`.
